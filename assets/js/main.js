...
if (mobileMenus?.length) {
  const mobileMenuToggleButtons = document.querySelectorAll(".menu-bar button");
  ...
  document.querySelectorAll(".mobile-menu li a").forEach(link => {
    link.addEventListener("click", () => {
      // إزالة الكلاس "active" من جميع القوائم
      mobileMenus.forEach(menu => menu.classList.remove("active"));
      // إزالة الكلاس "active" من كل أزرار القائمة
      mobileMenuToggleButtons.forEach(btn => btn.classList.remove("active"));
    });
  });
}
